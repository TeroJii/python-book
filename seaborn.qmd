...
Correlation is a measure of the strength and direction of a linear relationship between two numerical variables. For example, for the numeric columns in the `penguins` dataset, we can calculate the correlation matrix using the `corr()` function. @tbl-pen-corr shows the correlation matrix for the `penguins` dataset.

```{python}
#| code-fold: false
#| label: tbl-pen-corr
#| tbl-cap: Correlation matrix for the numerical columns in the penguins dataset.
...
#| code-fold: false
#| label: fig-heatmap
#| fig-cap: Heatmap showing the correlation between numerical variables in the penguins dataset. Update Seaborn to display the correlation values...

sns.heatmap(pen_corr, annot=True)
```

The `annot` parameter is used to display the correlation values in the heatmap. The `cmap` parameter can be used to change the color map of the heatmap.
